...
let sortBtn = document.getElementById("bubble_sort");
...
function swapHeight(i, j) {
  ...
  async function selectionSort() {
    ...
    for (var i = 0; i < data.length - 1; i++) {
      let minIndex = i;
      changeColor(minIndex, "lightgreen");
      for (var j = i + 1; j < data.length; j++) {
        await delay();
        changeColor(j , "lightgreen");
        ...
        changeColor(j, "lightblue");
      }
      if (minIndex !== i) {
        swapNumber(i,minIndex);
        await delay();
      }
...
function delay() {
  ...
  const time = 10000 / document.getElementById("size").value;
